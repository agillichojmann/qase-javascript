...
class PlaywrightReporter implements Reporter {
  ...
  public constructor(_options: QaseOptions) {
    ...
    this.options.uploadAttachments =
      !!PlaywrightReporter.getEnv(Envs.uploadAttachments) || this.options.uploadAttachments;
  ...
  private static createHeaders() {
    ...
    const xClientHeader = `playwright=${playwrightVersion as string}; qase-playwright=${
      playwrightCaseReporterVersion as string
  ...
  public async onBegin(): Promise<void> {
    ...
    return this.checkProject(this.options.projectCode, async (prjExists): Promise<void> => {
      if (!prjExists) {
  ...
  public async onEnd(): Promise<void> {
    ...
    try {
      await this.api.results.createResultBulk(this.options.projectCode, Number(this.runId), body);
  ...
  private getCaseParameters(test: TestCase) {
  ...
  private async createRun(
    ...
    try {
      ...
      const environmentId =
        Number.parseInt(PlaywrightReporter.getEnv(Envs.environmentId)!, 10) || this.options.environmentId;

      const runObject = PlaywrightReporter.createRunObject(name || `Automated run ${new Date().toISOString()}`, [], {
        description: description || "Playwright automated run",
        environment_id: environmentId,
        is_autotest: true,
      });
      const res = await this.api.runs.createRun(this.options.projectCode, runObject);
      cb(res.data);
    } catch (err) {
  ...
  private async uploadAttachments(testResult: TestResult) {
    return await Promise.all(
      testResult.attachments.map(async (attachment) => {
        ...
        const response = await this.api.attachments.uploadAttachment(this.options.projectCode, [data], options);
  ...
  private prepareCaseResult(test: TestCase, testResult: TestResult, attachments: any[]) {
    ...
    const parameters = this.getCaseParameters(test);
    ...
    if (caseIds.length === 0) {
      ...
      caseIds.forEach((caseId) => {
        ...
        this.log(chalk`{gray Result prepared for publish: ${test.title} }${add}`);
      });
    }
  }
}
